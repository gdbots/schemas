...
test('FileId tests', (t) => {
  ...
  t.true(id instanceof FileId);
  t.true(id.equals(FileId.fromString(`${id}`)));
...
test('FileId generate tests', (t) => {
  const date = new Date();
  const id = FileId.create('image', 'jpg');
...
test('FileId invalid tests', (t) => {
  ...
  invalid.forEach((v) => {
    try {
      FileId.fromString(v);
      t.fail(`FileId created with invalid value [${v}]`);
    } catch (e) {
